...
import com.dungeoncode.ca.automa.rules.ParityFlipRule;
 ...
 * {@link CentralSquareInitializer}. This configuration is inspired by the PARITY-FLIP rule described in Chapter 6,
 * Section 6.2 of <i>Cellular Automata Machines: A New Environment for Modeling</i>.
 *
 * @see ParityFlipRule
 * @see CentralSquareInitializer
 */
public class ParityFlipConfiguration extends AbstractConfiguration<BooleanCell, BooleanState> {
     ...
     */
    public ParityFlipConfiguration() {
        super(
     ...
     * Creates a {@link Grid} with the given dimensions, initialized by {@link CentralSquareInitializer}
     * with a centered 17x17 square of active cells, and applies the {@link ParityFlipRule} for state updates.
     *
    ...
    public void configure(Automa<BooleanCell, BooleanState> automa, int width, int height, long intervalMillis) {
        ...
        Grid<BooleanCell, BooleanState> grid = new Grid<>(width, height, new CentralSquareInitializer(8));
        config.put("grid", grid);
        config.put("rule", new ParityFlipRule());
