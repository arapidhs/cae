...
import com.dungeoncode.ca.automa.rules.OneOutOfEightRule;
 ...
 *
 * @see OneOutOfEightRule
 * @see CentralSquareInitializer
 */
public class OneOutOfEightConfiguration extends AbstractConfiguration<BooleanCell, BooleanState> {
     ...
     */
    public OneOutOfEightConfiguration() {
        super(
                "1-Out-Of-8",
     ...
     * with a 3x3 central region of active cells, and applies the {@link OneOutOfEightRule} for state updates.
     *
    ...
    public void configure(Automa<BooleanCell, BooleanState> automa, int width, int height, long intervalMillis) {
        ...
        Grid<BooleanCell, BooleanState> grid = new Grid<>(width, height, new CentralSquareInitializer(1));
        config.put("grid", grid);
        config.put("rule", new OneOutOfEightRule());
