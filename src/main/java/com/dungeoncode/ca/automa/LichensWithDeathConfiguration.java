...
import com.dungeoncode.ca.automa.rules.LichensWithDeathRule;
 ...
 *
 * @see LichensWithDeathRule
 * @see CentralSquareInitializer
 */
public class LichensWithDeathConfiguration extends AbstractConfiguration<BooleanCell, BooleanState> {
     ...
     */
    public LichensWithDeathConfiguration() {
        super(
                "Lichens With Death",
     ...
     * with a 5x5 central region of random state cells, and applies the {@link LichensWithDeathRule} for state updates.
     *
    ...
    public void configure(Automa<BooleanCell, BooleanState> automa, int width, int height, long intervalMillis) {
        ...
        config.put("rule", new LichensWithDeathRule());
